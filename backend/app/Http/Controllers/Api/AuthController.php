...
class AuthController
{
    ...
    public function refresh(Request $request, AuthSessionManager $sessions): JsonResponse
    {
        ...
        try {
            ...
            if (! $user || ! $user->is_active) return response()->json(['message' => 'Unauthorized.'], 401);
    ...
    public function verifyEmail(Request $request, int $id, string $hash): JsonResponse
    {
        ...
        if ($user->hasVerifiedEmail()) return response()->json(['message' => 'Email address is already verified.']);
    ...
    public function logout(Request $request, AuthSessionManager $sessions): JsonResponse
    {
        ...
        try { auth('api')->logout(); } catch (\Throwable) { /* idempotent logout */ }
        auth('web')->logout();
